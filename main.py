...
def extract_fecg(data, qrs):
    # FastICA - TS - FastICA
    ica1 = analysis.fast_ica(data, 4, processing.tanh)
    processing.plot_record(ica1, time_range=(0, 1), qrs=qrs,
                           title=["Signal channel: {}".format(str(x)) for x in range(data.shape[0])],
                           xlabel='Time, s',
                           ylabel='Amplitude')
    ...
    processing.plot_record(subtracted, time_range=(0, 1), qrs=qrs,
